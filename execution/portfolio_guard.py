from __future__ import annotations

from dataclasses import dataclass

from modules.capital.decision import CapitalDecision


@dataclass(frozen=True)
class PortfolioExecutionGuard:
    """
    Phase 13.5 — Portfolio-Aware Execution Guard

    Applies CapitalDecision in a SAFE way:
    - scale-down only
    - no mutation of intent
    - no authority override
    """

    def apply(
        self,
        *,
        requested_notional: float,
        decision: CapitalDecision,
    ) -> float:
        """
        Returns the allowed notional after portfolio intelligence.
        """
        if not decision.approved:
            return 0.0

        allowed = min(requested_notional, decision.max_notional)

        # Absolute safety: never increase exposure
        return max(0.0, allowed)
