...
class Tradehull:
    ...
    def __init__(self, ClientCode: str, token_id: str):
        ...
        try:
            ...
            try:
                ...
                self.step_df.rename(
                    {"index": "Symbol", 0: "Applicable Step value"},
                    axis="columns",
                    inplace=True,
                )
            except Exception as e:
                print(
                    "step Value DF is not generated due to Error from NSE India site: ",
                    e,
                )
    ...
    def get_instrument_file(self):
        ...
        for item in os.listdir("Dependencies"):
            ...
            if (item.startswith("all_instrument")) and (
                current_date not in item.split(" ")[1]
            ):
    ...
    def order_placement(
        ...
        try:
            ...
            security_id = self.instrument_df[
                ...
                & (
                    self.instrument_df["SEM_EXM_EXCH_ID"]
    ...
    def kill_switch(self, status):
        ...
        headers = {"Content-Type": "application/json", "access-token": self.token_id}

        killswitch_response = requests.post(
            killswitch_url, headers=headers, params=params
        )
    ...
    def get_live_pnl(self):
        ...
        try:
            ...
            for pos_ in pos_book_dict:
                ...
                underlying = self.instrument_df[
                    ((self.instrument_df["SEM_SMST_SECURITY_ID"] == security_id))
                ].iloc[-1]["SEM_CUSTOM_SYMBOL"]
                closePrice = self.get_ltp(underlying)
                Total_MTM = (
                    float(pos_["daySellValue"]) - float(pos_["dayBuyValue"])
                ) + (int(pos_["netQty"]) * closePrice * float(pos_["multiplier"]))
    ...
    def get_historical_data(self, tradingsymbol, exchange, days):
        try:
            ...
            expiry_code = self.instrument_df[
                (
                    (self.instrument_df["SEM_TRADING_SYMBOL"] == tradingsymbol)
                    | (self.instrument_df["SEM_CUSTOM_SYMBOL"] == tradingsymbol)
                )
    ...
    def get_intraday_data(self, tradingsymbol, exchange, timeframe):
        try:
            ...
            ohlc = self.Dhan.intraday_minute_data(
                str(security_id), exchangeSegment, instrument_type
            )
            if len(ohlc["data"]) != 0:
                df = pd.DataFrame(ohlc["data"])
                if not df.empty:
        ...
        try:
            ...
            if type(name) != list:
                ...
                if not exchange_nfo and not exchange_bfo:
                    ...
                    exchange_eq = "NSE"
                exchange_segment = (
                    "NFO" if exchange_nfo else ("BFO" if exchange_bfo else exchange_eq)
                )
                exchange = (
                    exchange_index[name] if name in exchange_index else exchange_segment
                )
                name = [name]
                df = (
                    self.sheet.range("A1")
                ...
                if data.empty:
                    new_name = self.instrument_df[
                        (
                            (self.instrument_df["SEM_CUSTOM_SYMBOL"] == name[0])
                            | (self.instrument_df["SEM_TRADING_SYMBOL"] == name[0])
                        )
                    ].iloc[-1]["SEM_TRADING_SYMBOL"]
                    new_name = [new_name]
                    df = (
    ...
    def ATM_Strike_Selection(self, Underlying, Expiry):
        try:
            ...
            instrument_df["SEM_EXPIRY_DATE"] = pd.to_datetime(
                instrument_df["SEM_EXPIRY_DATE"], errors="coerce"
            )
            instrument_df["ContractExpiration"] = instrument_df[
                "SEM_EXPIRY_DATE"
            ].dt.date
            instrument_df["ContractExpiration"] = instrument_df[
                "ContractExpiration"
            ...
            ce_condition = (
                ...
                & (
                    (instrument_df["SEM_TRADING_SYMBOL"].str.contains(Underlying))
                    | (instrument_df["SEM_CUSTOM_SYMBOL"].str.contains(Underlying))
                )
                & (instrument_df["ContractExpiration"] == Expiry)
                & (instrument_df["SEM_OPTION_TYPE"] == "CE")
            )
            pe_condition = (
                (instrument_df["SEM_EXM_EXCH_ID"] == exchange)
                & (
                ...
                & (instrument_df["ContractExpiration"] == Expiry)
                & (instrument_df["SEM_OPTION_TYPE"] == "PE")
            )
    ...
    def cancel_all_orders(self) -> dict:
        try:
            ...
            trigger_pending_orders = orders.loc[
                ...
                & (orders["productType"] == product)
            ]
            open_orders = orders.loc[
                (orders["orderStatus"] == "TRANSIT")
                & (orders["productType"] == product)
            ]
            ...
            bought = positions_df.loc[
                (positions_df["netQty"] > 0) & (positions_df["productType"] == product)
            ]
            sold = positions_df.loc[
                (positions_df["netQty"] < 0) & (positions_df["productType"] == product)
            ]
            ...
            for index, row in bought.iterrows():
                ...
                order_details[tradingsymbol] = dict(
                    {"orderid": sell_order_id, "price": 0}
                )
                time.sleep(0.5)
    ...
    def get_option_greek(
        ...
        try:
            ...
            days_to_expiry = (
                datetime.datetime.strptime(expiry_date, "%d-%m-%Y").date()
            ...
            if scrip_type == "CE":
                civ = mibian.BS(
                    [asset_price, strike, interest_rate, days_to_expiry], callPrice=ltp
                )
                cval = mibian.BS(
                    [asset_price, strike, interest_rate, days_to_expiry],
                    volatility=civ.impliedVolatility,
                    callPrice=ltp,
                )
                if flag == "price":
                ...
                if flag == "all_val":
                    return {
                        "callPrice": cval.callPrice,
                        "callDelta": cval.callDelta,
                        "callDelta2": cval.callDelta2,
                        "callTheta": cval.callTheta,
                        "callRho": cval.callRho,
                        "vega": cval.vega,
                        "gamma": cval.gamma,
                    }
            ...
            if scrip_type == "PE":
                piv = mibian.BS(
                    [asset_price, strike, interest_rate, days_to_expiry], putPrice=ltp
                )
                pval = mibian.BS(
                    [asset_price, strike, interest_rate, days_to_expiry],
                    volatility=piv.impliedVolatility,
                    putPrice=ltp,
                )
                if flag == "price":
                ...
                if flag == "all_val":
                    return {
                        "callPrice": pval.putPrice,
                        "callDelta": pval.putDelta,
                        "callDelta2": pval.putDelta2,
                        "callTheta": pval.putTheta,
                        "callRho": pval.putRho,
                        "vega": pval.vega,
                        "gamma": pval.gamma,
                    }
    ...
    def get_expiry(self, underlying):
        try:
            instrument_df = self.instrument_df.copy()
            data = instrument_df[instrument_df["Name"] == underlying].sort_values(
                "ContractExpiration"
            )
            data = pd.DataFrame(
                pd.to_datetime(
                    data[data["ContractExpiration"] != "1"]["ContractExpiration"]
                ).dt.date.unique(),
                columns=["Expiry Dates"],
            )
    ...
    def check_expiry_date(self, underlying, Expiry):
        try:
            ...
            ].sort_values("ContractExpiration")
            date = pd.to_datetime(
                data[data["ContractExpiration"] != "1"]["ContractExpiration"]
            ).dt.date.unique()
            if Expiry in date:
    ...
    def get_bid_ask(self, name):
        try:
            strike_exchange = self.instrument_df.loc[
                self.instrument_df["Description"] == name
            ].iloc[0][["ExchangeSegment"]][0]
            data = instrument_df.loc[
                (instrument_df["ExchangeSegment"] == strike_exchange)
                & (instrument_df["Description"] == name)
            ]
            ...
            ltp_quote = self.xts2.get_quote(
                Instruments=instruments, xtsMessageCode=1501, publishFormat="JSON"
            )
    ...
    def get_data_for_single_script(self, names: list) -> dict:
        try:
            ...
            for name in names:
                try:
                    if (name in self.token_dict) and (
                        name not in self.token_and_exchange
                    ):
                        token = self.token_dict[name]["token"]
                        token_exchange = self.token_dict[name]["exchange"]
                        self.token_and_exchange[name] = {
                            "token": token,
                            "token_exchange": token_exchange,
                        }
                    elif name not in self.token_and_exchange:
                        token = self.instrument_df.loc[
                            self.instrument_df["Description"] == name
                        ].iloc[0][["ExchangeInstrumentID"]][0]
                        token_exchange = self.instrument_df.loc[
                            self.instrument_df["Description"] == name
                        ].iloc[0][["ExchangeSegment"]][0]
                        self.token_and_exchange[name] = {
                            "token": token,
                    ...
                    continue
                instrument = {
                    "exchangeSegment": str(self.segment_dict[token_exchange]),
                    "exchangeInstrumentID": str(token),
                }
                instruments.append(instrument)
            response = self.xts2.get_quote(
                Instruments=instruments, xtsMessageCode=1501, publishFormat="JSON"
            )
            return response
        except Exception as e:
            ...
            self.logger.exception(
                f"Exception in getting get data for single script as {e}"
            )
            traceback.print_exc()
            ...
            if response:
                if type(names) == list:
                    for i, data in enumerate(response["result"]["listQuotes"]):
                        ...
                        name = self.instrument_df.loc[
                            self.instrument_df["ExchangeInstrumentID"]
    ...
    def get_orderhistory(self, order_id):
        try:
            ...
            while flag == True:
                try:
                    time.sleep(1)
                    order_history = self.xts1.get_order_history(
                        appOrderID=order_id, clientID=self.client_code
                    )
    ...
    def modify_order(
        ...
        try:
            ...
            self.order_Type = {
                "LIMIT": self.xts1.ORDER_TYPE_LIMIT,
                "MARKET": self.xts1.ORDER_TYPE_MARKET,
                "STOPLIMIT": self.xts1.ORDER_TYPE_STOPLIMIT,
                "STOPMARKET": self.xts1.ORDER_TYPE_STOPMARKET,
            }
            product = {
                "MIS": self.xts1.PRODUCT_MIS,
                "NRML": self.xts1.PRODUCT_NRML,
                "CNC": "CNC",
            }
    ...
    def cancel_order(self, OrderID: str) -> None:
        try:
            response = self.xts1.cancel_order(
                appOrderID=OrderID,
                orderUniqueIdentifier="123abc",
